  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createTrip({
        ...form,
        what_to_expect: form.what_to_expect,
        destinations: parseList(form.destinations),          // allow newline/CSV
        highlights: parseList(form.highlights),              // ✅ single textarea → array
        images: form.images.filter(Boolean),
        days: form.days.map(d => ({
          ...d,
          highlights: parseList(d.highlights),               // ✅ single textarea → array
        })),
      });
      navigate("/itineraries");
    } catch (error) {
